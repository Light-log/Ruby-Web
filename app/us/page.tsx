...
export default function USPage() {
  return <main lang="en-US" className="relative"><Navbar />
    <section className="relative overflow-hidden pt-3 pb-20 md:pb-28"><div className="absolute left-1/2 top-0 -z-10 h-[520px] w-[520px] -translate-x-1/2 rounded-full bg-crimson/10 blur-[130px]" /><div className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8 2xl:max-w-[88rem] text-center">
    ...
    <section className="py-12 md:py-16"><div className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8 2xl:max-w-[88rem]"><div className="mx-auto max-w-3xl text-center"><h2 className="font-display text-3xl text-ivory md:text-4xl">The issue is rarely a lack of software</h2><p className="mt-4 text-ivory-dim">It is usually that no system represents the whole workflow, its rules, and the information the team needs to make decisions.</p></div><div className="mt-10 grid gap-5 md:grid-cols-3">{["The same data is copied between systems and spreadsheets.", "Key workflows depend on email follow-ups and individual memory.", "Teams cannot see exceptions, ownership, or pending work early enough."].map((item) => <Card key={item} className="p-7"><CheckCircle2 className="h-6 w-6 text-crimson" /><p className="mt-5 leading-relaxed text-ivory-dim">{item}</p></Card>)}</div></div></section>

    <section id="solutions" className="py-16 md:py-24"><div className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8 2xl:max-w-[88rem]"><div className="max-w-3xl"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-crimson-dark">Solutions</p><h2 className="mt-3 font-display text-3xl text-ivory md:text-4xl">Start with the constraint affecting your operation</h2></div><div className="mt-10 grid gap-5 md:grid-cols-2">{services.map(([slug, Icon]) => { const service = usServices[slug]; return <Card key={slug} className="flex flex-col p-7 md:p-8"><Icon className="h-7 w-7 text-crimson" /><h3 className="mt-5 font-display text-2xl text-ivory">{service.label}</h3><p className="mt-3 flex-1 text-sm leading-relaxed text-ivory-dim">{service.description}</p><Link className="mt-6 inline-flex items-center gap-2 font-semibold text-crimson hover:text-crimson-dark" href={`/us/${slug}`}>Explore this service <ArrowRight className="h-4 w-4" /></Link></Card>; })}</div></div></section>

    <section className="py-12 md:py-16"><div className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8 2xl:max-w-[88rem]"><div className="mx-auto max-w-3xl text-center"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-crimson-dark">Selected work</p><h2 className="mt-3 font-display text-3xl text-ivory md:text-4xl">Products built for real operational needs</h2></div><div className="mt-10 grid gap-5 md:grid-cols-2"><Card className="p-8"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-crimson-dark">Mobile product · Publicly available</p><h3 className="mt-3 font-display text-2xl text-ivory">Lazo</h3><p className="mt-3 leading-relaxed text-ivory-dim">A mobile application delivered by DEVRUBY and published on Google Play and the Apple App Store. It supports linked accounts, shared daily activities, emotional check-ins, and timely notifications for couples.</p><a className="mt-5 inline-flex items-center gap-2 font-semibold text-crimson hover:text-crimson-dark" href="https://play.google.com/store/apps/details?id=app.lazo.com" target="_blank" rel="noreferrer">View on Google Play <ArrowRight className="h-4 w-4" /></a></Card><Card className="p-8"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-crimson-dark">Windows + web operations product</p><h3 className="mt-3 font-display text-2xl text-ivory">Maintenance Check</h3><p className="mt-3 leading-relaxed text-ivory-dim">A connected maintenance-management product for medical equipment: a native Windows application schedules persistent alerts and records maintenance actions, while the web panel supports equipment records, QR activity, staff management, and company information.</p></Card></div><p className="mx-auto mt-6 max-w-3xl text-center text-sm text-ivory-muted">We publish only the information authorized for each project. No client results or metrics are implied where they have not been documented.</p></div></section>
