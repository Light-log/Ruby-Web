        ...
        <div className="grid gap-6 md:grid-cols-3">
          {projects.map((p, idx) => (
            <Magnetic key={`${p.title}-${idx}`}>
              <div
                className="animate__animated animate__fadeInUp h-full"
                style={{ animationDelay: `${idx * 90}ms` }}
              >
                <ShineBorder className="h-full rounded-2xl">
                  {/* ✅ altura fija para que TODOS queden del tamaño del card "Tu próximo proyecto" */}
                  <article className="group relative flex h-[560px] flex-col overflow-hidden rounded-2xl border border-white/8 bg-gunmetal-950/55 backdrop-blur-xl">
                    {/* Imagen */}
                    ...
                    {/* Contenido (flex para empujar CTA abajo y unificar altura visual) */}
                    <div className="flex flex-1 flex-col p-6">
                      {p.isNext ? (
                        <div className="flex flex-1 flex-col">
                          {/* Título arriba (como en tu screenshot) */}
                          <div className="mb-3">
                        ...
                        <>
                          ...
                          <div className="mb-4 grid grid-cols-2 gap-2">
                            {p.metrics.map((m) => (
                              <div
                                key={m.label}
                                className="rounded-lg border border-white/8 bg-white/3 px-2 py-1.5"
                              >
                                <div className="text-[10px] leading-tight text-white/55">
                                  {m.label}
                                </div>
                                <div className="font-display text-[12px] leading-tight text-white">
                                  {m.value}
                                </div>
                              </div>
                            ))}
