        ...
        <FadeIn>
          <div className="rounded-3xl border border-black/6 bg-white/60 px-8 py-10 backdrop-blur-xl overflow-hidden shadow-card">
            ...
            <div className="relative overflow-hidden">
              <div className="pointer-events-none absolute left-0 top-0 bottom-0 w-16 z-10 bg-gradient-to-r from-white/80 to-transparent" />
              <div className="pointer-events-none absolute right-0 top-0 bottom-0 w-16 z-10 bg-gradient-to-l from-white/80 to-transparent" />

              <div className={cn("flex w-[200%] gap-3 animate-marquee")}>
                {content.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-3 rounded-2xl border border-black/8 bg-dark-200/60 px-5 py-3 text-sm text-ivory-dim whitespace-nowrap transition-colors hover:border-black/15 hover:text-ivory"
                  >
