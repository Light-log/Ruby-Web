 ...
 * - Google Analytics 4 NO se carga hasta que el usuario acepta.
...
export function ConsentBanner() {
  ...
  React.useEffect(() => {
    try {
      ...
      /* localStorage no disponible: mostramos el banner */
    }
    setMounted(true);
  }, []);
  ...
  function choose(value: Exclude<Consent, null>) {
    try {
      ...
      /* ignoramos: la sesión seguirá sin medición */
    }
    setConsent(value);
  }
    ...
    <>
      ...
      {mounted && consent === null && (
        <div
          ...
          className="fixed bottom-0 left-0 z-[1000] w-full max-w-[340px] p-4 pb-[max(1rem,env(safe-area-inset-bottom))] animate-slide-up sm:p-5"
        >
          <div className="relative w-full rounded-2xl bg-white border border-black/8 shadow-card-hover">
            <div className="flex flex-col items-center px-6 pb-6 pt-9">
              <span className="relative mx-auto -mt-16 mb-6 grid h-20 w-20 place-items-center rounded-2xl bg-dark-200 border border-black/8 shadow-card">
                <svg
              ...
              <h2 className="mr-auto mb-2 font-display text-base font-semibold text-ivory">
                Tu privacidad nos importa
              </h2>

              <p className="mb-4 w-full text-sm leading-relaxed text-ivory-dim">
                Usamos cookies de análisis (Google Analytics) para medir y mejorar
                nuestro sitio. Solo se activan si las aceptas. Consulta nuestra{" "}
                <Link
                  href="/privacidad"
              ...
              <div className="flex w-full items-center justify-between gap-3">
                <button
                  ...
                  className="touch-manipulation rounded-lg px-3 py-2.5 text-sm font-semibold text-ivory-muted transition-colors hover:text-crimson focus:outline-none focus:ring-2 focus:ring-crimson/30"
                >
                  Rechazar
                </button>
                <button
                  ...
                  className="touch-manipulation rounded-lg bg-crimson px-7 py-2.5 text-sm font-semibold text-white shadow-glow transition-colors hover:bg-crimson-light focus:outline-none focus:ring-2 focus:ring-crimson/40"
                >
                  Aceptar
                </button>
              </div>
